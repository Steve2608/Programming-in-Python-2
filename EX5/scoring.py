...
def mse(target_array, prediction_array):
    if prediction_array.shape != target_array.shape:
        raise IndexError(
            f"Target shape is {target_array.shape} but prediction shape is {prediction_array.shape}")
    prediction_array, target_array = np.asarray(prediction_array, np.float64), np.asarray(
...
def scoring(prediction_file: str, target_file: str):
    ...
    if not all([isinstance(prediction, np.ndarray) and np.uint8 == prediction.dtype
        ...
        raise TypeError(
            "List of predictions contains elements which are not numpy arrays of dtype uint8")
